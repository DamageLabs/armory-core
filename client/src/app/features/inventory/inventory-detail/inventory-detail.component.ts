    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      @if (isLoading()) {
        <div class="bg-slate-800 rounded-xl border border-slate-700 p-12 text-center">
          ...
          <p class="text-slate-400">Loading item details...</p>
        </div>
      }
      ...
      @if (item(); as itemData) {
        ...
        <div class="flex items-start justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h1 class="text-3xl font-bold text-slate-100">{{ itemData.name }}</h1>
              @if (itemData.description) {
                <p class="mt-1 text-slate-400">{{ itemData.description }}</p>
              }
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-slate-800 p-6 rounded-xl border border-slate-700">
              <h2 class="text-lg font-semibold text-slate-100 mb-4">Item Information</h2>
              
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label class="block text-sm font-medium text-slate-400 mb-1">Name</label>
                  <p class="text-slate-100 font-medium">{{ itemData.name }}</p>
                </div>
                
                <div>
                  <label class="block text-sm font-medium text-slate-400 mb-1">Quantity</label>
                  <p class="text-slate-100 font-medium">{{ itemData.quantity | number }}</p>
                </div>
                
                @if (itemData.cost) {
                  <div>
                    <label class="block text-sm font-medium text-slate-400 mb-1">Cost</label>
                    <p class="text-slate-100 font-medium">{{ formatCurrency(itemData.cost) }}</p>
                  </div>
                  
                  <div>
                    <label class="block text-sm font-medium text-slate-400 mb-1">Total Value</label>
                    <p class="text-slate-100 font-medium">{{ formatCurrency(itemData.cost * itemData.quantity) }}</p>
                  </div>
                }
                ...
                @if (itemData.location) {
                  <div>
                    ...
                    <p class="text-slate-100 font-medium">{{ itemData.location }}</p>
                  </div>
                }
              </div>
              ...
              @if (itemData.description) {
                <div class="mt-6">
                  ...
                  <p class="text-slate-100">{{ itemData.description }}</p>
                </div>
              }
              ...
              @if (itemData.notes) {
                <div class="mt-6">
                  ...
                  <p class="text-slate-100">{{ itemData.notes }}</p>
                </div>
              }
            </div>

            <!-- Custom fields -->
            @if (hasCustomFields(itemData.custom_fields)) {
              <div class="bg-slate-800 p-6 rounded-xl border border-slate-700">
                <h2 class="text-lg font-semibold text-slate-100 mb-4">Additional Information</h2>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  @for (field of getCustomFieldEntries(itemData.custom_fields); track field.key) {
                    <div>
                      <label class="block text-sm font-medium text-slate-400 mb-1">{{ field.key }}</label>
                      <p class="text-slate-100">{{ field.value || '-' }}</p>
                    </div>
                  }
                </div>
              </div>
          ...
          <div class="space-y-6">
            ...
            <div class="bg-slate-800 p-6 rounded-xl border border-slate-700">
              ...
              <div class="space-y-4">
                <div class="flex justify-between">
                  ...
                  <span class="text-slate-100">{{ formatDate(itemData.created_at) }}</span>
                </div>
                
                <div class="flex justify-between">
                  <span class="text-slate-400">Updated</span>
                  <span class="text-slate-100">{{ formatDate(itemData.updated_at) }}</span>
                </div>
                
                @if (itemData.cost && itemData.quantity) {
                  <div class="border-t border-slate-700 pt-4">
                    <div class="flex justify-between">
                      <span class="text-slate-400">Individual Value</span>
                      <span class="text-slate-100">{{ formatCurrency(itemData.cost) }}</span>
                    </div>
                    <div class="flex justify-between font-medium">
                      <span class="text-slate-300">Total Value</span>
                      <span class="text-amber-400">{{ formatCurrency(itemData.cost * itemData.quantity) }}</span>
                    </div>
                  </div>
                }
              </div>
            ...
            <div class="bg-slate-800 p-6 rounded-xl border border-slate-700">
              ...
              <div class="space-y-3">
                ...
                <button
                  (click)="duplicateItem()"
                  class="block w-full text-center bg-slate-600 hover:bg-slate-500 text-slate-100 font-semibold px-4 py-3 rounded-lg transition-colors duration-200">
                  Duplicate
                </button>
              </div>
            </div>
