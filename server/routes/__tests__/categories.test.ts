...
const app = createApp(categoryRoutes, '/api/categories');
...
describe('categories routes', () => {
  ...
  describe('GET /', () => {
    ...
    it('filters by typeId', async () => {
      vi.mocked(db.queryAll).mockReturnValue([mockCategory]);
      const res = await request(app).get('/api/categories?typeId=1');
      expect(res.status).toBe(200);
  ...
  describe('POST /', () => {
    it('creates category', async () => {
      ...
      const res = await request(app).post('/api/categories').send({ name: 'LEDs', sortOrder: 1, inventoryTypeId: 1 });
      expect(res.status).toBe(201);
    });

    it('returns 400 for missing name', async () => {
      const res = await request(app).post('/api/categories').send({ sortOrder: 0 });
      expect(res.status).toBe(400);
    });

    it('returns 400 for duplicate', async () => {
      vi.mocked(db.queryOne).mockReturnValue({ id: 1 });
      const res = await request(app).post('/api/categories').send({ name: 'Resistors', inventoryTypeId: 1 });
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('PUT /reorder', () => {
    it('reorders categories', async () => {
      ...
      const res = await request(app).put('/api/categories/reorder').send({ orderedIds: [3, 1, 2] });
      expect(res.status).toBe(200);
    });

    it('returns 400 for invalid input', async () => {
      const res = await request(app).put('/api/categories/reorder').send({ orderedIds: 'bad' });
      expect(res.status).toBe(400);
    });
  });
